...
export const baseNow = new Date("2024-02-15T12:00:00Z");
...
const generateEvents = (
  ...
  for (let i = 0; i < totalEvents; i += 1) {
    ...
    if (!preferredLocations.includes(location.id) && rng() < 0.25 + traits.newLocationBias) {
...
export const peopleBase: PersonBase[] = peopleSeeds.map((seed) => ({
  id: seed.id,
  name: seed.name,
}));

export const badgeEvents: BadgeEvent[] = peopleSeeds.flatMap((seed) =>
  generateEvents(seed.id, seedFromString(seed.id), seed.traits),
);
